...
### Running
Parameters to run application.
    
- ONTOLOGY [unknown]
- international [false]
- load [true]
```sh
  ./gradlew run --args='--international=true --ONTOLOGY=hp'
```
...
### Verification
From the workflow step `Deploy Cloud Run` copy out url `https://hpoweb-__________run.app` of the invoke endpoint:
```
Invoke endpoint:
https://hpoweb-__________run.app
```

Call the api endpoint:
```
curl https://hpoweb-__________run.app/hpoweb
```